...
func (b *builder) createGoAsmWrapper(forwardName string) {
	...
	switch b.arch() {
	case "amd64":
		inlineAsm = llvm.InlineAsm(asmType, "movq %rsp, ${0}", "=r,r", false, false, 0)
	case "arm64":
		inlineAsm = llvm.InlineAsm(asmType, "mov ${0}, sp", "=r,r", false, false, 0)
	}
	...
	case "amd64":
		...
		inlineAsm = llvm.InlineAsm(asmType, "movq %rsp, ${0}", "=r,r,~{rbx},~{r12},~{r13},~{r14},~{r15},~{memory}", true, true, 0)
	case "arm64":
		// https://developer.arm.com/documentation/ihi0055/d/
		// Save x19-x28, d8-d15.
		inlineAsm = llvm.InlineAsm(asmType, "mov ${0}, sp", "=r,r,~{x19},~{x20},~{x21},~{x22},~{x23},~{x24},~{x25},~{x26},~{x27},~{x28},~{d8},~{d9},~{d10},~{d11},~{d12},~{d13},~{d14},~{d15},~{memory}", true, true, 0)
	}
	stackPointer = b.CreateCall(inlineAsm, []llvm.Value{alloca}, "")
...
func (b *builder) createGoAsmExport(forwardName string) {
	...
	params = append(params, llvm.ConstNull(b.i8ptrType)) // parentHandle
...
func (b *builder) createGoAsmSPForward(forwardName string, forwardFunction llvm.Value) {
	...
	asm := llvm.InlineAsm(asmType, asmString, "=r", false, false, 0)
